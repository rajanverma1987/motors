...
function NavTextLink({ href, label, pathname, className = "", onClick }) {
  const active = isNavActive(pathname, href);
  return (
    <Link
      href={href}
      onClick={onClick}
      className={`inline-flex min-h-11 items-center whitespace-nowrap px-2 py-2 text-sm font-medium transition-colors touch-manipulation ${
        active ? "text-primary" : "text-secondary hover:text-title"
      } ${className}`.trim()}
    >
      {label}
    </Link>
...
export default function Navbar() {
  ...
  return (
    <>
      <header className="marketing-navbar sticky top-0 z-50 border-b border-border bg-card pt-[env(safe-area-inset-top)] shadow-sm">
      ...
      <div className="md:hidden">
        ...
        <aside
          ...
          className={`marketing-navbar fixed inset-y-0 right-0 z-[110] flex h-dvh max-h-dvh w-[min(100%,20rem)] max-w-[100vw] flex-col border-l border-border bg-card pb-[env(safe-area-inset-bottom)] pr-[env(safe-area-inset-right)] pt-[env(safe-area-inset-top)] shadow-xl transition-[transform,visibility] duration-200 ease-out ${
            mobileOpen
