...
export default function SimpleSupportSection() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      await alert({ title: "Error", message: e.message || "Could not load tickets", variant: "danger" });
      setTickets([]);
    } finally {
      setLoading(false);
    }
  ...
  const openDetail = async (row) => {
    ...
    try {
      ...
      await alert({ title: "Error", message: e.message || "Could not load ticket", variant: "danger" });
      setDetailOpen(false);
    } finally {
  ...
  const handlePhotoInput = async (e) => {
    ...
    if (room <= 0) {
      await alert({
        title: "Error",
        message: `You can attach up to ${MAX_TICKET_PHOTOS} photos.`,
        variant: "danger",
      });
      return;
    }
  ...
  async function handleCreate(e) {
    e.preventDefault();
    if (!newSubject.trim()) {
      await alert({ title: "Error", message: "Subject is required.", variant: "danger" });
      return;
    }
    if (!newDescription.trim()) {
      await alert({ title: "Error", message: "Describe the issue.", variant: "danger" });
      return;
    }
    setSaving(true);
    try {
      ...
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || "Failed");
      await alert({ title: "Submitted", message: `Ticket ${data.ticket?.ticketRef || ""} submitted.` });
      closeCreateModal();
      load();
    } catch (err) {
  ...
  async function sendReply() {
    if (!selectedId || !replyText.trim()) {
      await alert({ title: "Error", message: "Enter a message.", variant: "danger" });
      return;
    }
    setSaving(true);
    try {
      ...
      setReplyText("");
      await alert({ title: "Sent", message: "Reply sent." });
      load();
    } catch (e) {
    ...
    <div className="flex h-full min-h-0 w-full min-w-0 flex-1 flex-col overflow-hidden">
      <div className="mb-4 shrink-0 border-b border-border pb-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <h2 className="text-lg font-semibold text-title">Support</h2>
