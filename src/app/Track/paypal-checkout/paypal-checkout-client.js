  ...
  useEffect(() => {
        ...
        .Buttons({
          ...
          createSubscription(_data, actions) {
            return actions.subscription.create({
              ...
              custom_id: String(checkoutToken || "").slice(0, 127),
            });
          },
          onApprove(data) {
            setStatus("saving");
            return fetch("/api/track/checkout/complete", {
              ...
              .then(async (res) => {
                ...
                window.location.assign("/Track?paypal=success");
              })
              .catch((err) => {
                if (!cancelled) {
                  setStatus("error");
            ...
            if (!cancelled) {
              ...
              setError(
                detail
                  ...
                  : "PayPal could not start checkout. Try again in Safari or Chrome."
              );
            }
          },
        })
    ...
    if (window.paypal) {
      ...
      const onErr = () => {
        if (!cancelled) {
          setStatus("error");
          setError("PayPal script was blocked. Try again later.");
        }
      };
  ...
  return (
    <main className="mx-auto flex min-h-[70vh] max-w-lg flex-col items-center justify-center px-6 py-16 text-center">
      ...
      <p className="mt-3 text-sm text-secondary">
        ${Number(IQMOTORTRACK_MONTHLY_USD).toFixed(2)} / month. Unlimited motors. Pay with PayPal, then return to the
        app.
      </p>
...
export default function TrackPaypalCheckoutClient(props) {
