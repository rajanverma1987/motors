...
const FORM_ID = "admin-diagram-template-form";

export default function AdminDiagramTemplatesPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/admin/diagram-templates${qs}`, {
        credentials: "include",
        cache: "no-store",
      });
  ...
  const handleUpload = async (file) => {
    ...
    try {
      ...
      fd.append("file", file);
      const res = await fetch("/api/admin/diagram-templates/upload", {
        method: "POST",
        credentials: "include",
        body: fd,
      });
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      const res = await fetch("/api/admin/diagram-templates", {
  ...
  const handleDelete = async (row) => {
    const ok = await confirm({
      ...
      message: `Remove "${row.name}"? This affects all shops.`,
      confirmLabel: "Delete",
      variant: "danger",
    });
    if (!ok) return;
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-8">
      <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-xl font-bold text-title sm:text-2xl">Diagram designs</h1>
          <p className="mt-1 text-sm text-secondary">
            Blank diagram templates available to every shop for job drawings.
          </p>
        </div>
      ...
      <Table columns={columns} data={items} loading={loading} emptyMessage="No diagram designs yet." />
      ...
      <Modal
        ...
        actions={
          <>
            ...
            <Button type="submit" form={FORM_ID} size="sm" variant="primary" disabled={saving || uploading || !imageUrl}>
              {saving ? "Saving…" : "Save"}
            </Button>
          </>
        }
