...
export async function DELETE(request, context) {
  try {
    ...
    await connectDB();
    const result = await SalesCommission.deleteOne({ _id: id, createdByEmail: owner });
    if (result.deletedCount === 0) return NextResponse.json({ error: "Not found" }, { status: 404 });

    return NextResponse.json({ ok: true });
  } catch (err) {
    console.error("Dashboard delete sales commission error:", err);
    return NextResponse.json({ error: err.message || "Failed to delete sales commission" }, { status: 500 });
  }
}
