...
function resolveOwnedFilePath(url, ownerKey, recordId) {
  ...
  if (!raw) return null;
  const prefix = `/uploads/simple-purchase-orders/${ownerKey}/${recordId}/`;
...
export async function POST(request, context) {
  ...
  try {
    ...
    if (!isValidSimplePortalId(recordId)) {
      return NextResponse.json({ error: "Valid purchase order id required" }, { status: 400 });
    }

    await connectDB();
    const email = user.email.trim().toLowerCase();
    const doc = await SimplePurchaseOrder.findOne({ _id: recordId, createdByEmail: email });
    if (!doc) {
    ...
    console.error("Simple purchase order upload error:", err);
...
export async function DELETE(request, context) {
  ...
  try {
    ...
    console.error("Simple purchase order attachment delete error:", err);
