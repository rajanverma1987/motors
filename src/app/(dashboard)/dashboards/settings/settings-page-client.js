...
import {
  SIMPLE_SETTINGS_DRAFT_SECTION_IDS,
  SIMPLE_SETTINGS_SECTIONS,
  resolveSimpleMasterTab,
...
export default function SettingsPageClient() {
  ...
  async function handlePasswordChange(e) {
    ...
    try {
      ...
      setPwNew("");
      setPwConfirm("");
      await alert({ title: "Updated", message: "Password updated." });
    } catch (err) {
      await alert({
        title: "Error",
  ...
  async function handleTestSmtp() {
    ...
    try {
      ...
      await alert({ title: "SMTP", message: d.message || "SMTP connection successful." });
    } catch (e) {
      await alert({ title: "Error", message: e.message || "SMTP test failed.", variant: "danger" });
    } finally {
  ...
  async function handleLogoFile(e) {
    ...
    try {
      ...
      await alert({
        title: "Logo updated",
        message: "It appears in the header and on quote/PO emails.",
      });
    } catch (err) {
      await alert({ title: "Error", message: err.message || "Could not upload logo.", variant: "danger" });
    } finally {
  ...
  async function handleRemoveLogo() {
    ...
    try {
      ...
      setDraft(mergeUserSettings(d.settings));
      await refreshContext();
      await alert({ title: "Removed", message: "Logo removed." });
    } catch (err) {
      await alert({ title: "Error", message: err.message || "Could not remove logo.", variant: "danger" });
    } finally {
      setLogoUploading(false);
    }
  }
  ...
  const settingsTabs = useMemo(
    () => [
      ...
      },
      {
        ...
        children: <SimpleApiIntegrationSection />,
      },
      {
        id: "subscription",
        label: "Subscription",
        children: <SimpleSubscriptionSection />,
      },
      {
        id: "support",
        label: "Support",
        children: <SimpleSupportSection />,
      },
    ],
    [
  ...
  const sectionContentById = useMemo(() => {
    const map = {};
    for (const tab of settingsTabs) map[tab.id] = tab.children;
    return map;
  }, [settingsTabs]);
